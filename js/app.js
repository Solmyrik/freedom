(() => {
    ...
    const scrollWidth = headerElement.clientWidth;
    if (scrollWidth > 991) {
        document.addEventListener("afterPopupOpen", (function(e) {
            headerElement.style.paddingRight = "8px";
        }));
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        var onPhoneInput = function(e) {
            ...
            if ([ "7", "8", "9", "1", "2", "3", "4", "5", "6", "0" ].indexOf(inputNumbersValue[0]) > -1) {
                if ("7" != inputNumbersValue[0]) inputNumbersValue = "7" + inputNumbersValue;
    ...
    const checkBox12 = document.querySelector(".checkbox-12");
    button1.addEventListener("click", (async () => {
        let currentName = nameInput1.value;
        let currentPhone = phoneInput1.value;
        let currentNumber = `Заявка Freedom(Rezident) id${getRandomInt(1e5)}`;
        if (currentPhone.length > 17 && currentName.length > 1 && true === checkBox1.checked && true === checkBox12.checked) {
            let formData = new FormData;
            formData.append("name1", currentName);
            formData.append("phone1", currentPhone);
            formData.append("number1", currentNumber);
    ...
    const checkBox22 = document.querySelector(".checkbox-22");
    button2.addEventListener("click", (async () => {
        let currentName = nameInput2.value;
        let currentPhone = phoneInput2.value;
        let currentNumber = `Заявка Freedom(Rezident) id${getRandomInt(1e5)}`;
        if (currentPhone.length > 17 && currentName.length > 1 && true === checkBox2.checked && true === checkBox22.checked) {
            let formData = new FormData;
            formData.append("name2", currentName);
            formData.append("phone2", currentPhone);
            formData.append("number2", currentNumber);
            console.log(currentName, currentPhone, currentNumber);
            await fetch("sendmail.php", {
                method: "POST",
                body: formData
    ...
    button3.addEventListener("click", (async () => {
        ...
        if (currentPhone.length > 17 && currentName.length > 1) {
            let formData = new FormData;
            formData.append("name3", currentName);
            formData.append("phone3", currentPhone);
            formData.append("number3", currentNumber);
            console.log(currentName, currentPhone, currentNumber);
            await fetch("sendmail.php", {
                method: "POST",
                body: formData
